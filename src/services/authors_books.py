...
class AuthorsBooksService(BaseService):
    ...
    async def create_author(self, data: AuthorCreate) -> AuthorResponse:
        ...
        try:
            result = await saga_orchestrator.start_saga(
            ...
            logger.info("Author created", extra={"entity_id":str(author.id), "request_id": self.request_id})
            return AuthorResponse.model_validate(author)

        except Exception as e:
            logger.error("Author creation failed: {e}", extra={"request_id": self.request_id}, exc_info=True)
            raise HTTPException(500, f"Failed to create author: {str(e)}")
    ...
    async def get_authors_with_bio_required(self, author_id: UUID) -> AuthorResponse:
        author = await self.author_repo.get_with_books(author_id)
        if not author:
            logger.warning("Author not found", extra={"author_id": str(author_id), "request_id": self.request_id})
            raise NotFoundError("Author", str(author_id))
        ...
        try:
            ...
            if not bio_data:
                ...
                raise HTTPException(404, f"Bio not found for author {author_id}")
            response.rating = bio_data.get("rating")
            response.awards_count = bio_data.get("awards_count")

        except BioServiceError as e:
            logger.error(f"Bio Service error: {e}", extra={"author_id": str(author.id), "request_id": self.request_id})
            raise HTTPException(503, f"Bio service is temporarily unavailable. Please try again later")
    ...
    async def delete_author(self, author_id: UUID) -> None:
        try:
            ...
            await redis_client.invalidate(f"author:{author_id}")
            logger.info("Author deleted", extra={"entity_id": str(author_id), "request_id": self.request_id})

        except Exception as e:
            logger.error(f"Failed to delete author: {e}", extra={"author_id": str(author_id), "request_id": self.request_id}, exc_info=True)
            raise HTTPException(500, f"Failed to delete author: {str(e)}")
